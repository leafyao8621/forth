...
if (parser->state & PARSER_STATE_INTERPRET) {\
    ...
    *(uintptr_t*)vm->interpreted_cur = n;\
...
if (parser->state & PARSER_STATE_COMPILE) {\
    ...
    *(uintptr_t*)vm->compiled_cur = n;\
    vm->compiled_cur += sizeof(uintptr_t);\
}\
return PARSER_STATUS_OK;

int parser_handler_sqlite_ok(ForthParser *parser, char **iter, ForthVM *vm) {
    TEMPLATE(SQLITE_OK)
}

int parser_handler_sqlite_done(ForthParser *parser, char **iter, ForthVM *vm) {
    TEMPLATE(SQLITE_DONE)
}
