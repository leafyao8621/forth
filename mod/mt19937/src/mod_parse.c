...
int parser_handler_mt19937_size(ForthParser *parser, char **iter, ForthVM *vm) {
    if (!iter) {
    ...
    if (parser->state & PARSER_STATE_INTERPRET) {
        ...
        *(uintptr_t*)vm->interpreted_cur = sizeof(struct MT19937);
    ...
    if (parser->state & PARSER_STATE_COMPILE) {
        ...
        *(uintptr_t*)vm->compiled_cur = sizeof(struct MT19937);
